...
public class PostgresAccounts {
	...
	public static void main(String[] args){
		...
		//Account newAccount = new Account(3, "Sammy Kaplan", "skaplan", "password123", "gskaplan@asu.edu", "1000 State St");
	...
	public static void createAccountsTable() {
		try {
			...
			"ID INT PRIMARY KEY NOT NULL," + // randomized 6-digit ID
	 ...
	 * First digit [1-9] all other digits [0-9]
	 * 
	...
	public static void addAccount(Account newAccount) {
		...
		try {
			...
			if (newAccount.getType() == 1) {
				...
				"ID,TYPE,NAME,USERNAME,PASSWORD,EMAIL,ADDRESS) " +
				"VALUES(" +
				newAccountID() + ", " +
				newAccount.getType() + ", " +
				"'" + newAccount.getName() + "', " +
	...
	/**
	 * 
	 * @param ID 6-digit numeric ID for the account
	 ...
	 * @param username
	 */
	public static void setAccountUsername(int ID, String username) {
		Statement stmt = null;
		try {
			c.setAutoCommit(false);
			stmt = c.createStatement();
			String sql = "UPDATE Accounts SET USERNAME = '" + username + "' WHERE ID = " + ID + ";";
			stmt.executeUpdate(sql);
			c.commit();
			stmt.close();
			System.out.println("Account ID = " + ID + " successfully updated");
